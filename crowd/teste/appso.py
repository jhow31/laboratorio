...
def cad_people():
	name = request.form['name_vendors']
	subname = request.form['subname_vendors']
	telephone = request.form['telephone_vendor']
	email = request.form['email_vendor']
	rg = request.form['rg_vendors']
	cpf = request.form['cpf_vendor']
	city = request.form['city_vendor']
	neighborhood = request.form['netighborhood_vendor']
	street = request.form['street_vendor']
	number = request.form['number_vendor']
	complement = request.form['complement_vendor']
	zip = request.form['zip_vendor']
	associ = request.form['associ_vendor']
	photo = request.form['fileToUpload']
	entrepreneurship_vendor = request.form['entrepreneurship_vendor']
	password = request.form['password_vendor']
	encoded = base64.b64encode(password)
	cur = mysql.connection.cursor()
	cur.execute("SELECT rg FROM peoples  WHERE rg  = %s;", [rg])
	data = cur.fetchone()
	print(data)
	if data is None:
		cur.execute("INSERT INTO peoples (nome, sobrenome, email, telefone, rg, cpf, cidade, bairro, rua, numero, complemento, cep, associacao, empresa, password, people_fc) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (name, subname, email, telephone, rg, cpf, city, neighborhood, street, number, complement, zip, associ, entrepreneurship_vendor, encoded, photo))
		cur.execute("SELECT nome FROM peoples  WHERE rg  = %s;", [rg])
...
def chart2():
	timestamp = datetime.datetime.now()
	time2 = timestamp  - datetime.timedelta(days=15)
	print(timestamp)
        chart = pygal.HorizontalBar()
        cur = mysql.connection.cursor()
        cur.execute("select produto, quantidade from produtos;")
	...
	x = []
	for x in documents:
#		print(x["Produto"],x["Quantidade"],x["Valor"],str(x["TimeStamp"]))
